...
createApp({
	...
	computed: {
		availableCoupons() {
			return this.coupons.filter(
				coupon => coupon.displayStatus === 'AVAILABLE'
			);
		},

		selectedCoupon() {
			return this.availableCoupons.find(
				coupon =>
					coupon.memberCouponId
					=== this.form.memberCouponId
			) || null;
		},
		...
		bookingNights() {
			...
			const checkIn = new Date(
				`${this.form.checkInDate}T00:00:00`
			);

			const checkOut = new Date(
				`${this.form.checkOutDate}T00:00:00`
			);

			const nights = Math.round(
				(checkOut - checkIn) / 86400000
			);
		...
		estimatedTotal() {
			if (this.bookingNights <= 0) {
				return 0;
			}

			return this.form.rooms.reduce(
				(total, room) => {
		...
		estimatedDiscount() {
			...
			if (coupon.discountType === 2) {
				const payPercent =
					Number(coupon.discountPercent) || 100;

				const paidAfterDiscount =
					Math.round(
						total * payPercent / 100
					);

				return Math.max(
					0,
					total - paidAfterDiscount
				);
			}
	...
	methods: {
		async loadCoupons() {
		    ...
		    try {
		        this.coupons =
		            await this.api('/api/member/coupons');

		        const selectedStillAvailable =
		            this.availableCoupons.some(
		                coupon =>
		                    coupon.memberCouponId
		                    === this.form.memberCouponId
		            );
		...
		couponDiscountText(coupon) {
		    ...
		    if (coupon.discountType === 1) {
		        ...
		        return `折抵 $${amount.toLocaleString()}`;
		    }
		...
		async createOrder() {
		    ...
		    try {
		        ...
		        try {
		            this.confirmDetail =
		                await this.api(
		                    `/thestar/order/member/order/detail/${r.orderId}`
		                );
